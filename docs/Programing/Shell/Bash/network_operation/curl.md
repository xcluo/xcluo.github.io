...
- `-H header_json` --header 添加请求头，多个请求头需要以`-H`开头多行表示  
- `-L` --location 自动跟随http重定向  
- `-d escape_json` --data 发送 POST 数据，其中@和<字符存在转义，即从文件中读取  
- `--data-raw escape_json` 去除了@和<两特殊字符的处理，其余与--data一致
- `-o file_name`, --output 将输出保存为新文件  
- `-O`, --remote-name 使用远程文件名保存
- `--output-dir output_dir` 指定文件存放目录
- `-s` --silent 静默模式

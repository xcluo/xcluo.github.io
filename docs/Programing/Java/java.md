---
title: Java
---

- 通过查看jdk文件夹下的release文件，快速查看java版本号
- junit @Test 测试用例函数单独运行
- MyBatis映射机制
- lambda函数：CompletableFuture.supplyAsync(() -> dashboardService.getAppealOverview(req));
- Lombok
- HikariDataSource：数据库连接池，提高数据库连接效率

### 多线程

#### 线程池

- [ExecutorService](./libs/thread_pool/#executorservice)

### 进阶：文件读写

#### 数据格式

- [JSONObject](./libs/file_format/#json)：json格式数据处理

#### 输入输出流

- [IOUtils](./libs/iostream/#ioutils)：IO工具类
- [Bufferedreader](./libs/iostream/#bufferedreader)：缓冲输入流

### 进阶：数据库操作

#### MyBatis

